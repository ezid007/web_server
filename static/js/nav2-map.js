...
class Nav2MapViewer {
    ...
    renderRobot() {
        ...
        const screenY =
            this.offsetY + (this.mapInfo.height - robotY) * this.scale;
    ...
    sendGoal(screenX, screenY) {
        ...
        console.log(
            `🎯 Goal 전송: (${worldX.toFixed(2)}, ${worldY.toFixed(2)})`
        );
    ...
    autoFitMap() {
        ...
        // 수평 중앙, 수직 상단 정렬 (위쪽 빈 공간 제거)
        this.offsetX = (canvasWidth - mapWidth * this.scale) / 2;
        this.offsetY = padding; // 상단에 약간의 여백만 두고 배치

        console.log(
            `🗺️ 맵 자동 스케일: ${this.scale.toFixed(
                2
            )}x, 오프셋: (${this.offsetX.toFixed(0)}, ${this.offsetY.toFixed(
                0
            )})`
        );
    }
