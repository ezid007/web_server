...
> [!TIP] **면접관이 꼬리 질문으로 "양자화랑 LoRA가 같은 건가요?"라고 묻는다면?**
> "아닙니다, 엄밀히 다릅니다.
> **양자화(Quantization)**는 모델의 덩치(용량)를 줄이는 **'압축 기술'**이고,
> **LoRA**는 모델을 다 훈련하지 않고 일부만 훈련하는 **'학습 기법'**입니다.
> 저는 이 두 가지를 합친 **QLoRA** 기술을 사용하여, **[압축된 모델(4bit) + 가벼운 학습(LoRA)]** 조합으로 최적화했습니다."라고 대답하면 100점입니다.
